...
class TestSessionLifecycle:
    ...
    def test_active_session_in_kafka_not_postgres(self):
        ...
        session_id = "e2e_test_active_001"
        station_id = "charger1"
        ...
        producer.produce("ocpp.messages_test", key=session_id, value=start_msg)
        ...
        producer.produce("ocpp.messages_test", key=session_id, value=meter_msg)
        producer.flush(timeout=5)
        
        # Wait for Spark pipeline to process (this may take time)
        time.sleep(10)
        
        # Check: Session should be in ocpp.active_test
        consumer = Consumer({
        ...
        # Check: Session should be in ocpp.active.raw_test
        consumer = Consumer({
            "bootstrap.servers": TEST_KAFKA_BROKER,
        ...
        consumer.subscribe(["ocpp.active.raw_test"])
        ...
        # Check: Session should NOT be in PostgreSQL ocpp_history_test
        conn = psycopg2.connect(TEST_POSTGRES_URL, connect_timeout=5)
        cursor = conn.cursor()
        cursor.execute('SELECT 1 FROM ocpp_history_test WHERE sessionid = %s', (session_id,))
        in_postgres = cursor.fetchone() is not None
        conn.close()
        
        assert found_in_active, "Active session should be in ocpp.active_test"
        assert found_in_raw, "Active session should be in ocpp.active.raw_test"
        assert not in_postgres, "Active session should NOT be in ocpp.history_test"

    def test_completed_session_in_postgres_not_kafka(self):
        """Completed sessions should be in PostgreSQL and removed from Kafka."""
        producer = Producer({"bootstrap.servers": TEST_KAFKA_BROKER})
        
        session_id = "e2e_test_completed_001"
        station_id = "charger2"
        ...
        producer.produce("ocpp.messages_test", key=session_id, value=stop_msg)
        producer.flush(timeout=5)
        ...
        conn = psycopg2.connect(TEST_POSTGRES_URL, connect_timeout=5)
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM ocpp_history_test WHERE sessionid = %s', (session_id,))
        pg_session = cursor.fetchone()
        conn.close()
        
        assert pg_session is not None, "Completed session should be in PostgreSQL ocpp_history_test"
        
        # Check: Session should NOT be in ocpp.active_test (tombstoned)
        consumer = Consumer({
            "bootstrap.servers": TEST_KAFKA_BROKER,
            "group.id": "e2e_test_completed_active_check",
            "auto.offset.reset": "earliest",
            "enable.auto.commit": False
        })
        consumer.subscribe(["ocpp.active_test"])
        ...
        # Check: Session should NOT be in ocpp.active.raw_test (tombstoned)
        consumer = Consumer({
            "bootstrap.servers": TEST_KAFKA_BROKER,
            "group.id": "e2e_test_completed_raw_check",
            "auto.offset.reset": "earliest",
            "enable.auto.commit": False
        })
        consumer.subscribe(["ocpp.active.raw_test"])
        ...
        assert not found_in_active, "Completed session should NOT be in ocpp.active_test (tombstoned)"
        assert not found_in_raw, "Completed session should NOT be in ocpp.active.raw_test (tombstoned)"
...
class TestSessionDataCorrectness:
    ...
    def test_session_data_populated_correctly(self):
        ...
        cursor.execute('''
            ...
            FROM ocpp_history_test WHERE sessionid = %s
        ''', (session_id,))
        ...
        # Unpack the session data (PostgreSQL uses lowercase column names)
        ...
        assert db_station_id == station_id
        assert db_transaction_id == transaction_id
        assert db_meter_start == meter_start
        assert db_meter_stop == meter_stop
        assert db_reason == "RemoteStop"
        assert db_event_count >= 4  # Start + 3 MeterValues + Stop
        assert db_duration == 300  # 5 minutes = 300 seconds
        assert db_energy is not None and db_energy > 0
