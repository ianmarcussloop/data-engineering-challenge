...
from compacted test topics. They use topics that are auto-created
...
class TestTombstoneBehavior:
    ...
    def test_tombstone_removes_from_ocpp_active(self):
        """Sending tombstone should be accepted by ocpp.active."""
    ...
    def test_tombstone_removes_from_ocpp_active_raw(self):
...
class TestTombstoneProduction:
    ...
    def test_can_produce_tombstone_to_active(self):
        """Should be able to produce tombstone (null value) to ocpp.active."""
        producer = Producer({"bootstrap.servers": TEST_KAFKA_BROKER})
    ...
    def test_can_produce_tombstone_to_active_raw(self):
        """Should be able to produce tombstone (null value) to ocpp.active.raw."""
        producer = Producer({"bootstrap.servers": TEST_KAFKA_BROKER})
        session_id = "tombstone_prod_raw_001"
        
        # Produce should not raise an exception
        producer.produce("ocpp.active.raw", key=session_id, value=None)
        producer.flush(timeout=5)
